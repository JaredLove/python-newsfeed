...
# Note that db.add_all() alone doesn't change the database. In fact, it only preps the SQL queries. 
# To run the INSERT statements, you need to call db.commit(). If you don't need to make any other 
# database transactions at this time, you can close the session connection by calling db.close().
